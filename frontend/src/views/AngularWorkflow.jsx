...
import Gsoc2CodeEditor from "../components/Gsoc2CodeEditor.jsx";
...
// look for keys and set values with gsoc2 dotnotation 
	...
	const handleColoring = (actionId, status, label) => {
		...
		switch (status) {
			case "EXECUTING":
				...
				currentnode.removeClass("gsoc2-hover-highlight");
				...
				if (outgoingEdges.length > 0) {
					for (let i = 0; i < outgoingEdges.length; i++) {
						...
						if (
							...
							targetnode.removeClass("gsoc2-hover-highlight");
...
//console.log(responseJson)
  ...
  const sendStreamRequest = (body) => {
	...
	const streamUrl = "https://stream.gsoc2r.io"
  ...
  const saveWorkflow = (curworkflow, executionArgument, startNode) => {
    ...
    if (isCloud && !isLoggedIn) {
      ...
      window.location.href = `/register?view=/workflows/${props.match.params.key}&message=You need sign up to use workflows with Gsoc2`;
      return;
    }
      ...
      .then((responseJson) => {
        ...
        if (!responseJson.success) {
          ...
          if (responseJson.reason !== undefined && responseJson.reason !== null) {
            ...
            toast("Failed to save. Please contact your support@gsoc2r.io or your local admin if this is unexpected.")
          }
        } else {
  ...
  //"Testing",
  const internalIds = ["Gsoc2 Tools", "http", "email"];
      ...
      .then((responseJson) => {
        ...
        if (responseJson.success === false) {
          return
				...
				const foundTools = responseJson.find((app) => app.name === "Gsoc2 Tools")
  ...
  const startWorkflowStream = async (workflowId) => {
	...
	//const streamUrl = "https://gsoc2-streaming-backend-stbuwivzoq-ew.a.run.app"
	  //
	const streamUrl = "https://stream.gsoc2r.io"
      ...
      .then((responseJson) => {
        ...
        if (sourcenode.id !== undefined) {
          ...
          var triggers = responseJson.triggers.map((trigger) => {
            ...
            if (trigger.app_name !== "Gsoc2 Workflow" && trigger.app_name !== "User Input") {
              return null
            }
      ...
      */
      if (
        ...
        if (curaction !== undefined && curaction !== null) {
          if (
            curaction.app_name === "Gsoc2 Tools" &&
  ...
  const onCtxTap = (event) => {
    ...
    if (nodedata.type === "TRIGGER" && (nodedata.app_name === "Gsoc2 Workflow" || nodedata.app_name === "User Input")) {
  ...
  const onNodeDragStop = (event, selectedAction) => {
    ...
    if (
      ...
      ((nodedata.app_name !== "Gsoc2 Tools" &&
        nodedata.app_name !== "Testing" &&
        nodedata.app_name !== "Gsoc2 Workflow" &&
  ...
  const onNodeSelect = (event, newAppAuth) => {
    ...
    if (data.isSuggestion === true) {
	  ...
	  if (newAction.app_name === "Gsoc2 Subflow") {
	...
	ReactDOM.unstable_batchedUpdates(() => {
         ...
         if (data.buttonType === "suggestion") {
          ...
          if (parentNode !== null && parentNode !== undefined && parentNode.data() !== undefined && parentNode.data() !== null) {
            ...
            if (
              ...
              newNodeData.app_name !== "Gsoc2 Workflow"
            ) {
            } else {
      ...
      if (data.type === "ACTION") {
        ...
        if (data.app_name === "Gsoc2 Workflow" || data.app_name === "User Input") {
  ...
  const onEdgeAdded = (event) => {
    ...
    if (sourcenode === undefined || sourcenode === null || destinationnode === undefined || destinationnode === null) {
      ...
      if (sourcenode.data("type") === "TRIGGER") {
        if (sourcenode.data("app_name") !== "Gsoc2 Workflow" && sourcenode.data("app_name") !== "User Input") {
          setTimeout(() => {
    ...
    if (targetnode !== -1) {
      ...
      if (workflow.triggers[targetnode].app_name === "User Input" || workflow.triggers[targetnode].app_name === "Gsoc2 Workflow" || workflow.triggers[targetnode].app_name === "Gsoc2 Subflow") {
      } else {
    ...
    for (let branchkey in workflow.branches) {
      if (
        ...
        if (targetnode !== -1) {
          console.log("TARGETNODE: ", targetnode)
          if (workflow.triggers[targetnode].app_name === "User Input" || workflow.triggers[targetnode].app_name === "Gsoc2 Workflow") {
          } else {
            toast("Can't have triggers as target of branch")
            event.target.remove()
            found = true
  ...
  const onNodeAdded = (event) => {
    ...
    if (nodedata.type === "ACTION") {
      ...
      */
      ...
      if (nodedata.app_name === "Gsoc2 Tools") {
        const iconInfo = GetIconInfo(nodedata);
        const svg_pin = `<svg width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" version="1.1" xmlns="http://www.w3.org/2000/svg"><path d="${iconInfo.icon}" fill="${iconInfo.iconColor}"></path></svg>`;
        const svgpin_Url = encodeURI("data:image/svg+xml;utf-8," + svg_pin);
	  ...
	  if (edgeToBeAdded.data.source !== edgeToBeAdded.data.target && edgeToBeAdded.data.source !== undefined && edgeToBeAdded.data.target !== undefined) {
		  if (nodedata.name !== "User Input" && nodedata.name !== "Gsoc2 Workflow") {
		...
		for (let nodekey in allNodes) {
			...
			if (curnode.data.type !== "TRIGGER") {
			  ...
			  if (curnode.data.id === edge.data("source")) {
				...
				if (curnode.data.app_name !== "Gsoc2 Workflow" && curnode.data.app_name !== "User Input") {
  ...
  const onNodeHoverOut = (event) => {
    ...
    if ((nodedata.app_name === "Testing" || nodedata.app_name === "Gsoc2 Tools") && !nodedata.isStartNode) {
      parsedStyle = {
        "border-width": "1px",
        "font-size": "0px",
      };
    ...
    if (parentNode.data("isButton") || parentNode.data("buttonId")) return;
		...
		for (let recKey in workflowRecommendations) {
			...
			for (let recIndex in rec.recommendations) {
				...
				// Find out if foundApp is gsoc2 tools, and if so, add the correct image based on name
  ...
  const onEdgeHover = (event) => {
    ...
    if (event.target !== undefined && event.target !== null) {
      ...
      //event.target.addClass("gsoc2-hover-highlight");
  ...
  const setupGraph = (inputworkflow) => {
    ...
    const actions = inputworkflow.actions.map((action) => {
      ...
      if (!action.isStartNode && action.app_name === "Gsoc2 Tools") {
    ...
    const decoratorNodes = inputworkflow.actions.map((action) => {
      if (!action.isStartNode) {
        if (action.app_name === "Testing") {
          return null;
        } else if (action.app_name === "Gsoc2 Tools") {
          return null;
        }
      }
  ...
  const setNewSelectedAction = (e) => {
    ...
    if (newaction === undefined || newaction === null) {
      toast("Failed to find the action you selected. Please try again or contact support@gsoc2r.io if it persists.");
      return;
    }
	...
	if (oldaction.parameters !== undefined && oldaction.parameters !== null && newSelectedAction.parameters !== undefined && oldaction.id === newSelectedAction.id) {
    	...
    	if (newSelectedAction.app_name === "Gsoc2 Tools") {
  ...
  const getParents = (action) => {
    ...
    results = results.filter((data) => data.type === "ACTION" || data.app_name === "Gsoc2 Workflow" || data.app_name === "User Input");
  ...
  const AppConditionHandler = (props) => {
    ...
    if (actionlist.length === 0) {
      ...
      actionlist.push({
        ...
        name: "Gsoc2 DB",
        value: "$gsoc2_cache",
        highlight: "gsoc2_cache",
        autocomplete: "gsoc2_cache",
        example: "tmp",
      })
        ...
        helperText={
          data.value !== undefined &&
            ...
            <span style={{ color: "white", marginBottom: 5, marginleft: 5 }}>
              Use "Gsoc2 Tools" app with "Filter List" action to handle loops
            </span>
          ) : null
        }
        onBlur={(e) => {
                ...
                const handleExecArgumentHover = (inside) => {
                  ...
                  if (
                    ...
                    for (let triggerkey in workflow.triggers) {
                      ...
                      if (node.length > 0) {
                        if (inside) {
                          node.addClass("gsoc2-hover-highlight");
                        } else {
                          node.removeClass("gsoc2-hover-highlight");
                        }
                      }
                    }
                  }
              ...
              <span>
                ...
                {triggerFolders.length === 0 ?
                  <Typography variant="body2">
                    No folders found. Please authenticate first. If this persists, <a href="https://gsoc2r.io/contact" rel="noopener noreferrer" target="_blank" style={{ textDecoration: "none", color: "#f86a3e" }}>contact us</a>.
                  </Typography>
                  :
                  <Select
                    MenuProps={{
    ...
    const ActionlistWrapper = (props) => {
      ...
      const handleItemClick = (values) => {
        ...
        */
        ...
        if (selectedTrigger.name === "Gsoc2 Workflow") {
  ...
  const WorkflowMenu = () => {
    ...
    const [showGsoc2Menu, setShowGsoc2Menu] = React.useState(false);
        ...
        >
          <span>
            <Button
              ...
              variant="outlined"
              onClick={(event) => {
                setShowGsoc2Menu(!showGsoc2Menu);
  ...
  const allowList = ["gsoc2", "m1nk-code", "DavidtheGoliath"]
  ...
  const getExecutionSourceImage = (execution) => {
	...
	} else if (execution.execution_source === "Gsoc2GPT") {
      return (
		<AutoAwesomeIcon 
		  color="secondary" 
                  ...
                  data.workflow.actions !== undefined &&
                	...
                	for (let triggerkey in data.workflow.triggers) {
                	  ...
                	  if (
                	    ...
                	    (trigger.app_name === "Gsoc2 Workflow" &&
        ...
        <div style={{ padding: isMobile ? "0px 10px 25px 10px" : "25px 15px 25px 15px", maxWidth: isMobile ? "100%" : 400, overflowX: "hidden" }}>
                ...
                <div style={{}}>
                  ...
                  {environments.length > 0 && defaultEnvironmentIndex < environments.length && nonskippedResults.length === 0 && environments[defaultEnvironmentIndex].Name !== "Cloud" ?
                    <Typography variant="body2" color="textSecondary" style={{}}>
                      No results yet. Is Orborus running for the "{environments[defaultEnvironmentIndex].Name}" environment? <a href="/admin?tab=environments" rel="noopener noreferrer" target="_blank" style={{ textDecoration: "none", color: "#f86a3e" }}>Find out here</a>. If the Workflow doesn't start within 30 seconds with Orborus running, contact support: <a href="mailto:support@gsoc2r.io" rel="noopener noreferrer" target="_blank" style={{ textDecoration: "none", color: "#f86a3e" }}>support@gsoc2r.io</a>
                    </Typography>
                    : null}
                </div>
              ) : null}
          ...
          </div>
          {
            ...
            executionData.results.map((data, index) => {
              ...
              if (
                data.action.app_name === "Gsoc2 Tools" &&
              ...
              return (
                <div
                  ...
                  onMouseOver={() => {
                    ...
                    if (currentnode !== undefined && currentnode !== null && currentnode.length !== 0) {
                      currentnode.addClass("gsoc2-hover-highlight");
                    }
                ...
                >
                  <div style={{ display: "flex" }}>
                    ...
                    {data.action.app_name === "gsoc2-subflow" &&
  ...
  const curapp = !codeModalOpen
    ? {}
    : selectedResult.action.app_name === "gsoc2-subflow"
      ? triggers[2]
        ...
        <div style={{ display: "flex", marginBottom: 15 }}>
          {curapp === null ? null : (
            <img
              ...
              src={selectedResult === undefined ? theme.palette.defaultImage : selectedResult.action.app_name === "gsoc2-subflow" ? triggers[4].large_image : selectedResult.action.app_name === "User Input" ? triggers[5].large_image : selectedResult.action !== undefined && selectedResult.action.large_image !== undefined && selectedResult.action.large_image !== null && selectedResult.action.large_image !== "" ? selectedResult.action.large_image : curapp !== undefined ? curapp.large_image : theme.palette.defaultImage}
              style={{
                marginRight: 20,
                width: imgsize,
                height: imgsize,
            ...
            <span
              ...
              onClick={() => {
                ...
                if (copyText !== null && copyText !== undefined) {
                  ...
                  console.log(
                    "Failed to copy. copy_element_gsoc2 is undefined"
                  );
                }
              }}
            >
