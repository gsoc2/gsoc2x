...
const Workflows = (props) => {
  ...
  document.title = "Gsoc2 - Workflows";
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(
    "https://github.com/gsoc2/gsoc2-workflows"
  );
      ...
      for (var key in data.triggers) {
        const trigger = data.triggers[key];
				if (skip_sanitize !== true) {
					if (trigger.app_name === "Gsoc2 Workflow") {
        ...
        trigger.environment = isCloud ? "cloud" : "Gsoc2";
        trigger.id = newId;
      }
    }
  ...
  const publishWorkflow = (data) => {
    ...
    // This ALWAYS talks to Gsoc2 cloud
  ...
  const NewWorkflowPaper = () => {
	...
	const getWorkflowAppgroup = (data) => {
		...
		for (var key in data.actions) {
			...
			if (parsedAction.app_name === "Gsoc2 Tools" || parsedAction.app_id === "bc78f35c6c6351b07a09b7aed5d29652") {
				continue
			}
  ...
  const getWorkflowMeta = (data) => {
    ...
    if (
      ...
      for (let key in data.triggers) {
        if (data.triggers[key].app_name === "Gsoc2 Workflow") {
          subflows += 1;
        }
      }
    }
  ...
  const workflowButtons = (
    <span>
      ...
      {isCloud ? null : (
        <Tooltip color="primary" title={"Import workflows to Gsoc2"} placement="top">
          <Button
            color="secondary"
            style={{}}
            variant="text"
  ...
  const WorkflowView = () => {
    if (workflows.length === 0) {
      ...
      //      <div>
      //        <h2>Welcome to Gsoc2</h2>
      //      </div>
      //      <div>
      //        <p>
      //          <b>Gsoc2</b> is a flexible, easy to use, automation platform
          ...
          actionImageList.length > 0 ? (
            ...
            >
              {actionImageList.map((data, index) => {
                ...
                if (data.app_name.toLowerCase() === "gsoc2 tools") {
    ...
    >
      ...
      <DialogContent style={{ color: "rgba(255,255,255,0.65)" }}>
        ...
        <TextField
          ...
          placeholder="https://github.com/gsoc2/gsoc2-apps"
          fullWidth
        />
