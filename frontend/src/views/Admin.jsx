...
If you're interested, please let me know a time that works for you, or set up a call here: https://drift.me/${username}`
  ...
  const setConfig = (event, inputValue) => {
    ...
    setCurTab(newValue);
    if (newValue === 1) {
      document.title = "Gsoc2 - admin - users";
      getUsers();
    } else if (newValue === 2) {
      document.title = "Gsoc2 - admin - app authentication";
      getAppAuthentication();
    } else if (newValue === 3) {
      document.title = "Gsoc2 - admin - Files";
    } else if (newValue === 4) {
      document.title = "Gsoc2 - admin - Datastore";

      //listOrgCache("3fd181b9-fb29-41b7-b2f5-15292265d420");
    } else if (newValue === 5) {
      document.title = "Gsoc2 - admin - schedules";
      getSchedules();
    } else if (newValue === 6) {
      document.title = "Gsoc2 - admin - environments";
      getEnvironments();
    } else if (newValue === 7) {
      document.title = "Gsoc2 - admin - orgs";
      getOrgs();
    } else {
        ...
        {selectedOrganization.id === undefined ? (
          ...
          <div>
            ...
            />
            		...
            		do? Cloud syncronization is a way of getting more out of Gsoc2.
            		Gsoc2 will <b>ALWAYS</b> make every option open source, but
            		...
            		{isCloud ? (
            		  ...
            		  <div>
            		    ...
            		    {orgSyncResponse.length > 0 ? (
            		      <Typography style={{ marginTop: 5, marginBottom: 10 }}>
            		        Message from Gsoc2 Cloud: <b>{orgSyncResponse}</b>
            		      </Typography>
            		    ) : null}
            		  </div>
            		)}
    ...
    curTab === 8 ? (
      <div>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          ...
          <span style={{ marginLeft: 25 }}>
            Categories are the categories supported by Gsoc2, which are mapped
