...
const AppCreator = (defaultprops) => {
        ...
        for (let [method, methodvalue] of Object.entries(pathvalue)) {
          ...
          if (path.includes("/")) {
            ...
            for (let splitkey in pathsplit) {
							if (pathsplit[splitkey].includes("_gsoc2_replace_")) {
								const regex = /_gsoc2_replace_\d/i;
								//console.log("NEW: ", 
                ...
                if (
                  ...
                  if (
                    ...
                    if (methodvalue.responses.default.content["text/plain"]["schema"]["format"] === "binary" && methodvalue.responses.default.content["text/plain"]["schema"]["type"] === "string") {
                  		newaction.example_response = "gsoc2_file_download"
										}
                  }
                }
              }
          ...
          for (let paramkey in methodvalue.parameters) {
            ...
            if (parameter.in === "query") {
              ...
              // E.G: https://raw.githubusercontent.com/owentl/Gsoc2/master/gosecure.yaml
  ...
  const submitApp = () => {
    ...
    if (basedata.info !== undefined && basedata.info.contact !== undefined) {
      ...
      data.info["contact"] = {
        name: "@Anonymous Gsoc2 User",
        url: "https://twitter.com/gsoc2io",
        email: "support@soc2.khulnasoft.com",
      };
    } else {
    ...
    for (let actionkey in actions) {
      ...
      if (item.name === undefined && item.description !== undefined) {
        item.name = item.description;
			...
			if (handledPaths.includes(pathjoin)) {
				...
				// Max 100 of same lol
				for (let i = 0; i < 100; i++) {
					item.url = item.url+"_gsoc2_replace_"+i
      ...
      if (item.example_response !== undefined && item.example_response !== null && item.example_response.length > 0) {
				...
				if (item["example_response"] === "gsoc2_file_download") {
      ...
      if (item.queries.length > 0) {
        ...
        for (let querykey in item.queries) {
          ...
          var newitem = {
            in: "query",
            name: queryitem.name,
            description: "Generated by soc2.khulnasoft.com OpenAPI",
        ...
        for (let querykey in paths) {
      	...
      	if (
      	  ...
      	  item.body.length > 0
					...
					// This is to stop it from messing with systems in Gsoc2.
      	  ...
      	  data.paths[item.url][item.method.toLowerCase()]["requestBody"] = {
      	    description: "Generated by soc2.khulnasoft.com",
      	    required: required,
      	    content: {
      	      example: {
      	        example: item.body,
        ...
        for (let headerkey in headersSplit) {
          ...
          if (innerkey.length > 0 && value.length > 0) {
            newitem = {
              ...
              name: innerkey,
              multiline: false,
              description: "Header generated by soc2.khulnasoft.com OpenAPI",
              required: false,
              example: value,
              schema: {
                type: "string",
    ...
    authenticationOption === "Oauth2" ? (
      <div style={{ color: "white", marginTop: 20 }}>
        ...
        <Typography
          ...
          style={{ marginTop: 10 }}
        >
					Find the Authorization URL, Token URL and scopes in question for the API. Ensure the app in question is pointed at https://soc2.khulnasoft.com/set_authentication
        </Typography>
  ...
  const apiKey =
	...
	const ActionPaper = (props) => {
  	...
  	const getActionErrors = () => {
    		...
    		>
    		  <FormControl style={{ backgroundColor: surfaceColor, color: "white" }}>
    		    ...
    		    <DialogContent>
    		      <a
    		        target="_blank"
    		        href="https://soc2.khulnasoft.com/docs/app_creation#actions"
    		        ...
    		        <Button
    		          ...
    		          onClick={() => {
    		            ...
    		            if (fileDownloadEnabled) {
    		              ...
    		              setActionField("example_response", "gsoc2_file_download");
										}
    		            //setUpdate(Math.random());
    		          }}
    		        >
		...
		const url = data.url;
		const hasFile = (data["file_field"] !== undefined && data["file_field"] !== null && data["file_field"].length > 0) || data["example_response"] === "gsoc2_file_download"
        ...
        <div>
          <DialogContent>
            <a
              target="_blank"
              rel="norefferer"
              href="https://soc2.khulnasoft.com/docs/app_creation#authentication"
              style={{ textDecoration: "none", color: "#f85a3e" }}
            >
              What is this?
            </a>
  ...
  const testView = (
    <div style={{ color: "white" }}>
      ...
      <a
        target="_blank"
        href="https://soc2.khulnasoft.com/docs/app_creation#testing"
        style={{ textDecoration: "none", color: "#f85a3e" }}
      >
  ...
  const landingpageDataBrowser = (
    <div style={{ paddingBottom: 100, color: "white" }}>
      ...
      <Paper style={boxStyle}>
        ...
        <a
          target="_blank"
          href="https://soc2.khulnasoft.com/docs/app_creation#app-creator-instructions"
          style={{ textDecoration: "none", color: "#f85a3e" }}
        >
