...
gsoc2_filters = FilterManager()
for key, value in standard_filter_manager.filters.items():
    gsoc2_filters.filters[key] = value
...
#print(gsoc2_filters.filters)
#print(Liquid("{{ '10' | plus: 1}}", filters=gsoc2_filters.filters).render())
#print(Liquid("{{ '10' | minus: 1}}", filters=gsoc2_filters.filters).render())
#print(Liquid("{{ asd | size }}", filters=gsoc2_filters.filters).render())
#print(Liquid("{{ 'asd' | md5 }}", filters=gsoc2_filters.filters).render())
#print(Liquid("{{ 'asd' | sha256 }}", filters=gsoc2_filters.filters).render())
#print(Liquid("{{ 'asd' | md5_base64 | base64_decode }}", filters=gsoc2_filters.filters).render())
...
class AppBase:
    ...
    def __init__(self, redis=None, logger=None, console_logger=None):#, docker_client=None):
        ...
        if not os.getenv("GSOC2_LOGS_DISABLED") == "true":
    ...
    def send_result(self, action_result, headers, stream_path):
        ...
        try:
            log_contents = "disabled: add env GSOC2_LOGS_DISABLED=true to Orborus to re-enable logs for apps. Can not be enabled natively in Cloud except in Hybrid mode."
            if not os.getenv("GSOC2_LOGS_DISABLED") == "true":
            ...
            for param in action_result["action"]["parameters"]:
                if param["name"] == "gsoc2_action_logs": 
                    param_found = True
                    break

            if not param_found:
                action_result["action"]["parameters"].append({
                    "name": "gsoc2_action_logs",
                    "value": log_contents,
                })
    ...
    # Currently only working on local version of Gsoc2
    ...
    def execute_action(self, action):
        ...
        try:
            ...
            for param in tmp_parameters:
                if param["value"] == "GSOC2_AUTO_REMOVED":
                    replace_params = True
        except KeyError:
            action["parameters"] = []
        except TypeError:
        ...
        def get_json_value(execution_data, input_data):
            ...
            elif actionname_lower.startswith("gsoc2_cache ") or actionname_lower.startswith("gsoc2_db "): 
                actionname_lower = "gsoc2_cache"
            ...
            try: 
                ...
                elif actionname_lower == "gsoc2_cache":
            ...
            if is_loop:
                ...
                if parsersplit[-1] == "#":
                    ...
                    parseditem = "${GSOC2_NO_SPLITTER%s}$" % json.dumps(data)
                else:
        ...
        def parse_liquid(template, self):
            ...
            try:
                ...
                if "${" in template and "}$" in template:
                    self.logger.info("[DEBUG] Gsoc2 loop shouldn't run in liquid. Data length: %d" % len(template))
                    return template
                    ...
                    if len(action["parameters"]) < 1:
                        ...
                        for parameter in action["parameters"]:
                            ...
                            if len(actualitem) > 0:
                                ...
                                if len(actualitem[0]) > 2 and actualitem[0][1] == "GSOC2_NO_SPLITTER":
        ...
        # https://ptb.discord.com/channels/747075026288902237/882017498550112286/882043773138382890
            ...
            self.action_result["result"] = json.dumps({
                "success": False,
                "reason": f"General exception in the app. See gsoc2 action logs for more details.",
                "details": e,
            })
    ...
    def run(cls, action=""):
        ...
        exposed_port = os.getenv("GSOC2_APP_EXPOSED_PORT", "")
