...
func createSchedule(ctx context.Context, scheduleId, workflowId, name, startNode, frequency, orgId string, body []byte) error {
	...
	job := func() {
		...
		_, _, err := handleExecution(workflowId, gsoc2.Workflow{ExecutingOrg: gsoc2.OrgMini{Id: orgId}}, request, orgId)
		if err != nil {
...
func handleGetWorkflowqueueConfirm(resp http.ResponseWriter, request *http.Request) {
	...
	executionRequests, err := gsoc2.GetWorkflowQueue(ctx, id, 100)
	if err != nil {
	...
	var removeExecutionRequests gsoc2.ExecutionRequestWrapper
	...
	err = gsoc2.DeleteKeys(ctx, parsedId, ids)
	if err != nil {
	...
	//	err := gsoc2.SetWorkflowQueue(ctx, newExecutionRequests, id)
	//	if err != nil {
...
// (especially since we have a default: gsoc2)
func handleGetWorkflowqueue(resp http.ResponseWriter, request *http.Request) {
	...
	if len(environment) == 0 {
		...
		/*
			...
			resp.Write([]byte(fmt.Sprintf(`{"success": false, "reason": "Specify the org header. This can be done by setting the 'ORG' environment variable for Orborus to your Org ID in Gsoc2"}`)))
			return
		*/
	}
	...
	if request.Method == "POST" {
		if rand.Intn(1) == 0 {
			...
			if err == nil {
				...
				var envData gsoc2.OrborusStats
				...
				if err == nil && !envData.Swarm && !envData.Kubernetes && (envData.CPU > 0 || envData.Memory > 0 || envData.Disk > 0) {
					...
					if err == nil {
						...
						if len(percentages) > concurrentChecks {
							...
							if sendAlert {
								...
								err = gsoc2.CreateOrgNotification(
									ctx,
								...
								org, err := gsoc2.GetOrg(ctx, environment)
								if err == nil {
									...
									if !foundRecommendation {
										...
										org, _ = gsoc2.AddPriority(*org, gsoc2.Priority{
										...
										org.Priorities = append([]gsoc2.Priority{org.Priorities[len(org.Priorities)-1]}, org.Priorities[:len(org.Priorities)-1]...)
					...
					if len(percentages) > 0 {
						...
						// Add the new data
						go gsoc2.SetCache(ctx, cacheKey, cacheData, 5)
					}
				}
				...
				go gsoc2.SetenvStats(ctx, envData)
			}
		}
	}

	executionRequests, err := gsoc2.GetWorkflowQueue(ctx, orgId, 100)
	if err != nil {
	...
	if len(executionRequests.Data) == 0 {
		executionRequests.Data = []gsoc2.ExecutionRequest{}
	} else {
		...
		if len(env.Id) == 0 && len(env.Name) == 0 {
			...
			for _, requestData := range executionRequests.Data {
				execution, err := gsoc2.GetWorkflowExecution(ctx, requestData.ExecutionId)
				if err == nil {
			...
			if len(orgId) > 0 {
				env, err := gsoc2.GetEnvironment(ctx, orgId, foundId)
				if err != nil {
...
func handleGetStreamResults(resp http.ResponseWriter, request *http.Request) {
	...
	for _, action := range workflowExecution.Workflow.Actions {
		...
		cache, err := gsoc2.GetCache(ctx, cacheId)
		if err != nil {
...
func handleWorkflowQueue(resp http.ResponseWriter, request *http.Request) {
	...
	ctx := context.Background()
	err = gsoc2.ValidateNewWorkerExecution(ctx, body)
	if err == nil {
		resp.WriteHeader(200)
...
func runWorkflowExecutionTransaction(ctx context.Context, attempts int64, workflowExecutionId string, actionResult gsoc2.ActionResult, resp http.ResponseWriter) {
	...
	if setExecution || workflowExecution.Status == "FINISHED" || workflowExecution.Status == "ABORTED" || workflowExecution.Status == "FAILURE" {
		err = gsoc2.SetWorkflowExecution(ctx, *workflowExecution, true)
		//err = gsoc2.SetWorkflowExecution(ctx, *workflowExecution, dbSave)
		if err != nil {
			resp.WriteHeader(401)
...
func handleExecutionStatistics(execution gsoc2.WorkflowExecution) {
	// FIXME: CLEAN UP THE JSON THAT'S SAVED.
	// https://github.com/gsoc2/Gsoc2/issues/172
	appResults := []gsoc2.AppExecutionExample{}
	for _, result := range execution.Results {
		...
		if appFound {
			...
			executionExample := gsoc2.AppExecutionExample{
...
func deleteWorkflow(resp http.ResponseWriter, request *http.Request) {
	...
	for _, item := range workflow.Triggers {
		if item.TriggerType == "SCHEDULE" && item.Status != "uninitialized" {
			...
			err = gsoc2.HandleOutlookSubRemoval(ctx, user, workflow.ID, item.ID)
			if err != nil {
				log.Printf("[DEBUG] Failed to delete OUTLOOK email sub (checking gmail after): %s", err)
			}

			err = gsoc2.HandleGmailSubRemoval(ctx, user, workflow.ID, item.ID)
			if err != nil {
	...
	err = gsoc2.DeleteKey(ctx, "workflow", fileId)
	if err != nil {
...
func handleExecution(id string, workflow gsoc2.Workflow, request *http.Request, orgId string) (gsoc2.WorkflowExecution, string, error) {
	//go func() {
	...
	if workflow.ID == "" || workflow.ID != id {
		tmpworkflow, err := gsoc2.GetWorkflow(ctx, id)
		if err != nil {
			//log.Printf("[WARNING] Failed getting the workflow locally (execution setup): %s", err)
			return gsoc2.WorkflowExecution{}, "Failed getting workflow", err
		}
	...
	/*
		if len(workflow.ExecutingOrg.Id) == 0 {
			if len(orgId) > 0 {
				...
				return gsoc2.WorkflowExecution{}, fmt.Sprintf("Workflow has no executing org defined"), errors.New("Workflow has no executing org defined")
			}
		}
	*/

	if len(workflow.Actions) == 0 {
		workflow.Actions = []gsoc2.Action{}
	} else {
	...
	if len(workflow.Branches) == 0 {
		workflow.Branches = []gsoc2.Branch{}
	}
	if len(workflow.Triggers) == 0 {
		workflow.Triggers = []gsoc2.Trigger{}
	} else {
		newtriggers := []gsoc2.Trigger{}
	...
	if !workflow.IsValid {
		...
		return gsoc2.WorkflowExecution{}, fmt.Sprintf(`workflow %s is invalid`, workflow.ID), errors.New("Failed getting workflow")
	}
	...
	if err != nil {
		...
		return gsoc2.WorkflowExecution{}, "Failed unmarshal during execution", err
	}
	...
	if request.Method == "POST" {
		...
		if err != nil {
			...
			return gsoc2.WorkflowExecution{}, "Failed getting body", err
		}
		...
		if len(execution.Start) == 36 && len(workflow.Actions) > 0 {
			...
			if !found {
				...
				return gsoc2.WorkflowExecution{}, fmt.Sprintf("Startnode %s was not found in actions", workflow.Start), errors.New(fmt.Sprintf("Startnode %s was not found in actions", workflow.Start))
			}
		} else if len(execution.Start) > 0 {
			//log.Printf("[INFO] !")
			//log.Printf("[ERROR] START ACTION %s IS WRONG ID LENGTH %d!", execution.Start, len(execution.Start))
			//return gsoc2.WorkflowExecution{}, fmt.Sprintf("Startnode %s was not found in actions", execution.Start), errors.New(fmt.Sprintf("Startnode %s was not found in actions", execution.Start))
		}
		...
		if answerok && referenceok {
			...
			if answer[0] == "false" {
				...
				oldExecution, err := gsoc2.GetWorkflowExecution(ctx, referenceId[0])
				if err != nil {
					log.Printf("Failed getting execution (execution) %s: %s", referenceId[0], err)
					return gsoc2.WorkflowExecution{}, fmt.Sprintf("Failed getting execution ID %s because it doesn't exist.", referenceId[0]), err
				}

				if oldExecution.Workflow.ID != id {
					log.Println("Wrong workflowid!")
					return gsoc2.WorkflowExecution{}, fmt.Sprintf("Bad ID %s", referenceId), errors.New("Bad ID")
				}
				...
				err = gsoc2.SetWorkflowExecution(ctx, *oldExecution, true)
				if err != nil {
					log.Printf("Error saving workflow execution actionresult setting: %s", err)
					return gsoc2.WorkflowExecution{}, fmt.Sprintf("Failed setting workflowexecution actionresult in execution: %s", err), err
				}

				return gsoc2.WorkflowExecution{}, "", nil
			}
		}
	...
	if len(workflowExecution.ExecutionId) != 36 {
		...
		return gsoc2.WorkflowExecution{}, "Invalid uuid", err
	}
	...
	if !startnodeFound {
		...
		if len(newStartnode) > 0 {
			...
			return gsoc2.WorkflowExecution{}, fmt.Sprintf("Startnode couldn't be found"), errors.New("Startnode isn't defined in this workflow..")
		}
	}
	...
	for _, action := range workflowExecution.Workflow.Actions {
		...
		if len(workflowExecution.Results) > 0 {
			...
			if !found {
				...
				defaultResults = append(defaultResults, gsoc2.ActionResult{
					Action:        curaction,
	...
	for triggerIndex, trigger := range workflowExecution.Workflow.Triggers {
		...
		if trigger.AppName == "User Input" || trigger.AppName == "Gsoc2 Workflow" {
			...
			if !found {
				...
				//	if replaceActions {
				//		replacementNodes, newBranches, lastnode := gsoc2.GetReplacementNodes(ctx, workflowExecution, trigger, trigger.Label)
	...
	if !startFound {
		if len(workflowExecution.Start) == 0 && len(workflowExecution.Workflow.Start) > 0 {
			...
			return gsoc2.WorkflowExecution{}, fmt.Sprintf("Workflow action %s doesn't exist in workflow", workflowExecution.Start), errors.New(fmt.Sprintf(`Workflow start node "%s" doesn't exist. Exiting!`, workflowExecution.Start))
		}
	}
	...
	if len(workflowExecution.ExecutionOrg) > 0 {
		...
		allEnvironments, err := gsoc2.GetEnvironments(ctx, workflowExecution.ExecutionOrg)
		if err != nil {
			log.Printf("[WARNING] Failed finding environments: %s", err)
			return gsoc2.WorkflowExecution{}, fmt.Sprintf("Workflow environments not found for this org"), errors.New(fmt.Sprintf("Workflow environments not found for this org"))
		}
		...
		return gsoc2.WorkflowExecution{}, "No org identified for execution", errors.New("No org identified for execution")
	}

	if len(allEnvs) == 0 {
		log.Printf("[ERROR] No active environments found for org: %s", workflowExecution.ExecutionOrg)
		return gsoc2.WorkflowExecution{}, "No active environments found", errors.New(fmt.Sprintf("No active env found for org %s", workflowExecution.ExecutionOrg))
	}
	...
	for _, action := range workflowExecution.Workflow.Actions {
		...
		for _, env := range allEnvs {
			if env.Name == action.Environment {
				...
				if env.Type == "cloud" {
					...
					return gsoc2.WorkflowExecution{}, "No active environments found", errors.New(fmt.Sprintf("No handler for environment type %s", env.Type))
				}
				break
			}
		}
		...
		if !found {
			...
			return gsoc2.WorkflowExecution{}, "Couldn't find the environment", errors.New(fmt.Sprintf("Couldn't find env %s in org %s", action.Environment, workflowExecution.ExecutionOrg))
		}
	...
	if err != nil {
		...
		return gsoc2.WorkflowExecution{}, fmt.Sprintf("Failed setting workflowexecution: %s", err), err
	}
	...
	if execInfo.OnpremExecution {
		...
		for _, environment := range execInfo.Environments {
			...
			err = gsoc2.SetWorkflowQueue(ctx, executionRequest, environment)
			if err != nil {
	...
	if execInfo.CloudExec {
		...
		if len(workflowExecution.Workflow.Actions) == 1 {
			...
			return gsoc2.WorkflowExecution{}, "Cloud not implemented yet (1)", errors.New("Cloud not implemented yet")
		} else {
...
func cloudExecuteAction(execution gsoc2.WorkflowExecution) error {
	ctx := context.Background()
	org, err := gsoc2.GetOrg(ctx, execution.ExecutionOrg)
	if err != nil {
		return err
	}
	...
	type ExecutionStruct struct {
		...
		Action            gsoc2.Action         `json:"action" datastore:"action"`
		Authorization     string                 `json:"authorization" datastore:"authorization"`
		Results           []gsoc2.ActionResult `json:"results" datastore:"results,noindex"`
...
func executeWorkflow(resp http.ResponseWriter, request *http.Request) {
	...
	if userErr != nil {
		...
		executionAuthValid, newOrgId = gsoc2.RunExecuteAccessValidation(request, workflow)
	...
	user.ActiveOrg.Users = []gsoc2.UserMini{}
...
func stopSchedule(resp http.ResponseWriter, request *http.Request) {
	...
	schedule, err := gsoc2.GetSchedule(ctx, scheduleId)
	if err != nil {
	...
	if schedule.Environment == "cloud" {
		...
		if err != nil {
			...
			err = gsoc2.DeleteKey(ctx, "schedules", scheduleId)
			if err != nil {
...
func deleteSchedule(ctx context.Context, id string) error {
	...
	err := gsoc2.DeleteKey(ctx, "schedules", id)
	if err != nil {
...
func scheduleWorkflow(resp http.ResponseWriter, request *http.Request) {
	...
	if schedule.Environment == "cloud" {
		...
		err = gsoc2.SetSchedule(ctx, newSchedule)
		if err != nil {
...
func setExampleresult(ctx context.Context, result gsoc2.AppExecutionExample) error {
...
func getSpecificApps(resp http.ResponseWriter, request *http.Request) {
	...
	returnValues := []gsoc2.WorkflowApp{}
...
func handleAppHotloadRequest(resp http.ResponseWriter, request *http.Request) {
	...
	if len(location) == 0 {
		resp.WriteHeader(500)
		resp.Write([]byte(fmt.Sprintf(`{"success": false, "reason": "GSOC2_APP_HOTLOAD_FOLDER not specified in .env"}`)))
		return
	}
	...
	for _, file := range dir {
		...
		case mode.IsRegular():
			...
			if strings.Contains(filename, "yaml") || strings.Contains(filename, "yml") {
				...
				swagger, api, _, err := gsoc2.GenerateYaml(swagger, parsedOpenApi.ID)
				if err != nil {
				...
				if !found {
					err = gsoc2.SetWorkflowAppDatastore(ctx, api, api.ID)
					if err != nil {
						...
						err = gsoc2.SetOpenApiDatastore(ctx, parsedOpenApi.ID, parsedOpenApi)
						if err != nil {
	...
	for _, filename := range filenames {
		...
		case mode.IsRegular():
			...
			if strings.HasSuffix(filename, ".json") {
				...
				workflow.Org = append(workflow.Org, gsoc2.OrgMini{
					Id: orgId,
				})
				...
				*/
				...
				ctx := context.Background()
				err = gsoc2.SetWorkflow(ctx, workflow, workflow.ID, secondsOffset)
				if err != nil {
...
func setNewWorkflowApp(resp http.ResponseWriter, request *http.Request) {
	...
	var workflowapp gsoc2.WorkflowApp
	...
	ctx := context.Background()
	allapps, err := gsoc2.GetAllWorkflowApps(ctx, 1000, 0)
	if err != nil {
	...
	err = gsoc2.CheckWorkflowApp(workflowapp)
	if err != nil {
	...
	if workflowapp.Authentication.Required {
		...
		for index, action := range workflowapp.Actions {
			...
			appendParams := []gsoc2.WorkflowAppActionParameter{}
			for _, fieldname := range workflowapp.Authentication.Parameters {
				...
				if !found {
					appendParams = append(appendParams, gsoc2.WorkflowAppActionParameter{
						Name:          fieldname.Name,
...
func handleUserInput(trigger gsoc2.Trigger, organizationId string, workflowId string, referenceExecution string) error {
	// E.g. check email
	sms := ""
	email := ""
	subflow := ""
	...
	if strings.Contains(triggerType, "email") {
		...
		org, err := gsoc2.GetOrg(ctx, organizationId)
		if err != nil {
...
func executeSingleAction(resp http.ResponseWriter, request *http.Request) {
	...
	ctx := context.Background()
	workflowExecution, err := gsoc2.PrepareSingleAction(ctx, user, fileId, body)
	if err != nil {
		log.Printf("[INFO] Failed workflowrequest POST read: %s", err)
		resp.WriteHeader(401)
		resp.Write([]byte(`{"success": false}`))
	...
	returnBody := gsoc2.HandleRetValidation(ctx, workflowExecution, 1)
...
// Onlyname is used to
func IterateAppGithubFolders(ctx context.Context, fs billy.Filesystem, dir []os.FileInfo, extra string, onlyname string, forceUpdate bool) ([]gsoc2.BuildLaterStruct, []gsoc2.BuildLaterStruct, error) {
	var err error

	allapps := []gsoc2.WorkflowApp{}
	...
	buildLaterFirst := []gsoc2.BuildLaterStruct{}
	buildLaterList := []gsoc2.BuildLaterStruct{}
	for _, file := range dir {
		if len(onlyname) > 0 && file.Name() != onlyname {
			continue
		}
		...
		case mode.IsRegular():
			...
			if filename == "Dockerfile" {
				...
				workflowapp.ReferenceInfo.GithubUrl = fmt.Sprintf("https://github.com/gsoc2/gsoc2-apps/tree/master/%s/%s", strings.ToLower(newName), workflowapp.AppVersion)
				...
				if len(allapps) == 0 {
					allapps, err = gsoc2.GetAllWorkflowApps(ctx, 0, 0)
					if err != nil {
				...
				if len(removeApps) > 0 {
					for _, item := range removeApps {
						...
						err = gsoc2.DeleteKey(ctx, "workflowapp", item)
						if err != nil {
				...
				*/
				...
				buildLater := gsoc2.BuildLaterStruct{
					Tags:  tags,
					Extra: extra,
					Id:    workflowapp.ID,
				}
	...
	newSortedList := []gsoc2.BuildLaterStruct{}
	initApps := []string{
		"tools",
		"http",
		"email",
			...
			if err != nil {
				...
				if len(item.Tags) > 0 {
					err = gsoc2.CreateOrgNotification(
						...
						fmt.Sprintf("The app %s with image %s failed to build. Check backend logs with docker! docker logs gsoc2-backend", item.Tags[0], item.Extra),
						fmt.Sprintf("/apps"),
						orgId,
						false,
					)
	...
	if strings.Contains(tmpBody.URL, "github") || strings.Contains(tmpBody.URL, "gitlab") || strings.Contains(tmpBody.URL, "bitbucket") {
		...
		if tmpBody.ForceUpdate {
			...
			if err == nil {
				...
				appSdk := os.Getenv("GSOC2_APP_SDK_VERSION")
				if len(appSdk) == 0 {
					_, err := dockercli.ImagePull(ctx, "gsoc2/gsoc2:app_sdk", types.ImagePullOptions{})
					if err != nil {
						log.Printf("[WARNING] Failed to download new App SDK: %s", err)
					}
				} else {
					_, err := dockercli.ImagePull(ctx, fmt.Sprintf("%s/%s/gsoc2-app_sdk:%s", "ghcr.io", "gsoc2", appSdk), types.ImagePullOptions{})
					if err != nil {
...
func checkWorkflowApp(workflowApp gsoc2.WorkflowApp) error {
	// Validate fields
	if workflowApp.Name == "" {
