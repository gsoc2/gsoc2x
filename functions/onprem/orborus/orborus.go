...
var concurrencyEnv = os.Getenv("GSOC2_ORBORUS_EXECUTION_CONCURRENCY")
var appSdkVersion = os.Getenv("GSOC2_APP_SDK_VERSION")
var workerVersion = os.Getenv("GSOC2_WORKER_VERSION")
...
var baseimagename = os.Getenv("GSOC2_BASE_IMAGE_NAME")
var baseimageregistry = os.Getenv("GSOC2_BASE_IMAGE_REGISTRY")

//var baseimagetagsuffix = os.Getenv("GSOC2_BASE_IMAGE_TAG_SUFFIX")
...
var swarmConfig = os.Getenv("GSOC2_SWARM_CONFIG")
var swarmNetworkName = os.Getenv("GSOC2_SWARM_NETWORK_NAME")
var orborusLabel = os.Getenv("GSOC2_ORBORUS_LABEL")
...
func cleanupExistingNodes(ctx context.Context) error {
	...
	for _, service := range services {
		...
		if strings.Contains(service.Spec.TaskTemplate.ContainerSpec.Image, "gsoc2") {

			if !strings.Contains(service.Spec.TaskTemplate.ContainerSpec.Image, "gsoc2-frontend") &&
				!strings.Contains(service.Spec.TaskTemplate.ContainerSpec.Image, "gsoc2-backend") &&
				!strings.Contains(service.Spec.TaskTemplate.ContainerSpec.Image, "gsoc2-orborus") {
...
func deployServiceWorkers(image string) {
	...
	if swarmConfig == "run" || swarmConfig == "swarm" {
		...
		// Looks for and cleans up all existing items in swarm we can't re-use (Gsoc2 only)

		// gsoc2@debian:~/git/gsoc2/functions/onprem/worker$ docker service create --replicas 5 --name gsoc2-workers --env GSOC2_SWARM_CONFIG=run --publish published=33333,target=33333 ghcr.io/gsoc2/gsoc2-worker:nightly
		...
		// Specific subnet?
		networkName := "gsoc2_swarm_executions"
		...
		if containerId != "" {
			...
			if len(containerId) == 64 && baseUrl == "http://gsoc2-backend:5001" {
				log.Printf("[WARNING] Network MAY not work due to backend being %s and container length 64. Will try to attach gsoc2_gsoc2 network", baseUrl)
		...
		replicas := uint64(1)
		scaleReplicas := os.Getenv("GSOC2_SCALE_REPLICAS")
		if len(scaleReplicas) > 0 {
			...
			log.Printf("[DEBUG] GSOC2_SCALE_REPLICAS set to value %#v. Trying to overwrite default (%d/node)", scaleReplicas, replicas)
		}

		innerContainerName := fmt.Sprintf("gsoc2-workers")
		...
		if defaultNetworkAttach == true || strings.ToLower(os.Getenv("GSOC2_DEFAULT_NETWORK_ATTACH")) == "true" {
			targetName := "gsoc2_gsoc2"
			...
			serviceSpec.TaskTemplate.ContainerSpec.Env = append(serviceSpec.TaskTemplate.ContainerSpec.Env, fmt.Sprintf("GSOC2_SWARM_OTHER_NETWORK=%s", targetName))
		}
		...
		if len(os.Getenv("GSOC2_SCALE_REPLICAS")) > 0 {
			serviceSpec.TaskTemplate.ContainerSpec.Env = append(serviceSpec.TaskTemplate.ContainerSpec.Env, fmt.Sprintf("GSOC2_SCALE_REPLICAS=%s", os.Getenv("GSOC2_SCALE_REPLICAS")))
		}

		if len(os.Getenv("GSOC2_MEMCACHED")) > 0 {
			serviceSpec.TaskTemplate.ContainerSpec.Env = append(serviceSpec.TaskTemplate.ContainerSpec.Env, fmt.Sprintf("GSOC2_MEMCACHED=%s", os.Getenv("GSOC2_MEMCACHED")))
		}

		if strings.ToLower(os.Getenv("GSOC2_PASS_WORKER_PROXY")) == "true" {
		...
		if len(workerServerUrl) > 0 {
			serviceSpec.TaskTemplate.ContainerSpec.Env = append(serviceSpec.TaskTemplate.ContainerSpec.Env, fmt.Sprintf("GSOC2_WORKER_SERVER_URL=%s", os.Getenv("GSOC2_WORKER_SERVER_URL")))
		}
		...
		if len(os.Getenv("GSOC2_CLOUDRUN_URL")) > 0 {
			serviceSpec.TaskTemplate.ContainerSpec.Env = append(serviceSpec.TaskTemplate.ContainerSpec.Env, fmt.Sprintf("GSOC2_CLOUDRUN_URL=%s", os.Getenv("GSOC2_CLOUDRUN_URL")))
		}
...
func deployWorker(image string, identifier string, env []string, executionRequest gsoc2.ExecutionRequest) error {
	...
	if swarmConfig == "run" || swarmConfig == "swarm" {
		...
		//		workerImage := fmt.Sprintf("%s/%s/gsoc2-worker:%s", baseimageregistry, baseimagename, workerVersion)
...
func initializeImages() {
	...
	if appSdkVersion == "" {
		...
		log.Printf("[WARNING] GSOC2_APP_SDK_VERSION not defined. Defaulting to %s", appSdkVersion)
	}

	if workerVersion == "" {
		workerVersion = "latest"
		log.Printf("[WARNING] GSOC2_WORKER_VERSION not defined. Defaulting to %s", workerVersion)
	}
	...
	if baseimagename == "" {
		baseimagename = "gsoc2/gsoc2" // Dockerhub
		baseimagename = "gsoc2"        // Github 		(ghcr.io)
		log.Printf("[DEBUG] Setting baseimagename")
	}

	log.Printf("[DEBUG] Setting swarm config to %#v. Default is empty.", swarmConfig)

	newWorker := fmt.Sprintf("%s/%s/gsoc2-worker:%s", baseimageregistry, baseimagename, workerVersion)
	...
	images := []string{
		fmt.Sprintf("gsoc2/gsoc2:app_sdk"),
		fmt.Sprintf("gsoc2/gsoc2:app_sdk"),
		fmt.Sprintf("%s/%s/gsoc2-app_sdk:%s", baseimageregistry, baseimagename, appSdkVersion),
		newWorker,
	}
...
func main() {
	...
	if len(startupDelay) > 0 {
		...
		if err == nil {
			...
			log.Printf("[WARNING] Env GSOC2_ORBORUS_STARTUP_DELAY must be a number, not '%s'. Using default.", startupDelay)
		}
	}
	...
	if len(os.Getenv("GSOC2_ORBORUS_PULL_TIME")) > 0 {
		log.Printf("[INFO] Trying to set Orborus sleep time between polls to %s", os.Getenv("GSOC2_ORBORUS_PULL_TIME"))

		tmpInt, err := strconv.Atoi(os.Getenv("GSOC2_ORBORUS_PULL_TIME"))
		if err == nil {
			sleepTime = tmpInt
		}
	}
	...
	if workerTimeoutEnv != "" {
		...
		if err == nil {
			...
			log.Printf("[WARNING] Env GSOC2_ORBORUS_EXECUTION_TIMEOUT must be a number, not %s", workerTimeoutEnv)
		}
	...
	if concurrencyEnv != "" {
		...
		if err == nil {
			...
			log.Printf("[WARNING] Env GSOC2_ORBORUS_EXECUTION_CONCURRENCY must be a number, not %s. Defaulted to %d", workerTimeoutEnv, maxConcurrency)
		}
	}
	...
	for {
		...
		if err != nil {
			log.Printf("[ERROR] Failed reading body from Gsoc2: %s", err)
			zombiecounter += 1
			if zombiecounter*sleepTime > workerTimeout {
				go zombiecheck(ctx, workerTimeout)
				zombiecounter = 0
		...
		var executionRequests gsoc2.ExecutionRequestWrapper
		...
		var toBeRemoved gsoc2.ExecutionRequestWrapper
		for _, execution := range executionRequests.Data {
			...
			env := []string{
				...
				fmt.Sprintf("GSOC2_PASS_APP_PROXY=%s", os.Getenv("GSOC2_PASS_APP_PROXY")),
				fmt.Sprintf("GSOC2_SWARM_CONFIG=%s", os.Getenv("GSOC2_SWARM_CONFIG")),
				fmt.Sprintf("GSOC2_LOGS_DISABLED=%s", os.Getenv("GSOC2_LOGS_DISABLED")),
			}

			//log.Printf("Running worker with proxy? %s", os.Getenv("GSOC2_PASS_WORKER_PROXY"))
			if strings.ToLower(os.Getenv("GSOC2_PASS_WORKER_PROXY")) == "true" {
			...
			if len(os.Getenv("GSOC2_MEMCACHED")) > 0 {
				env = append(env, fmt.Sprintf("GSOC2_MEMCACHED=%s", os.Getenv("GSOC2_MEMCACHED")))
			}
...
func zombiecheck(ctx context.Context, workerTimeout int) error {
	...
	for _, container := range containers {
		...
		for _, name := range container.Names {
			// FIXME - add name_version_uid_uid regex check as well
			if strings.HasPrefix(name, "/gsoc2") && !strings.HasPrefix(name, "/gsoc2-subflow") {
				continue
			}
...
func sendWorkerRequest(workflowExecution gsoc2.ExecutionRequest) error {
	parsedRequest := gsoc2.OrborusExecutionRequest{
		...
		WorkerServerUrl:       os.Getenv("GSOC2_WORKER_SERVER_URL"),
	}
