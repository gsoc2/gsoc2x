...
try:
    with open("/tmp/gsoc2_start.txt", "w+") as tmp:
...
def main(args):
    ...
    try:
        with open("/tmp/gsoc2_end.txt", "w+") as tmp:
...
def filter_msg(alert):
    # These are things that recursively happen because Gsoc2 starts Docker containers
