...
echo "Starting docker registry with name gsoc2-local-registry-$random_string.."

docker run -d -p 5000:5000 --restart=always --name "gsoc2-local-registry-$random_string" \
  -v $(pwd)/certs:/certs \
